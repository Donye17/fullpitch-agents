"""NCR Agent — National Collegiate Rugby news and schedule/results ingest."""
...
def _sources(api: FullpitchAPI) -> tuple[str, str]:
    sources = [
        ...
        if "ncr" in source.get("name", "").lower()
    ]
    news_url = next((s["url"] for s in sources if s.get("type") == "news"), NCR_HOME_URL)
    schedule_url = next((s["url"] for s in sources if s.get("type") == "data"), NCR_SCHEDULE_URL)
...
def _ingest_article(api: FullpitchAPI, article: dict[str, str], client, summary: dict[str, Any]) -> None:
    try:
        ...
        logger.warning("Failed to fetch NCR article %s: %s", article["url"], exc)
        ...
        api.create_article(
            {
                ...
                "imageUrl": image_url,
                "agentName": "ncr-agent",
                "tags": ["college", "ncr", league],
            }
        )
        summary["articles_written"] += 1
    except FullpitchAPIError as exc:
        summary["errors"].append(f"Failed to ingest NCR article {article['title'][:60]}: {exc}")
...
def _ingest_result(api: FullpitchAPI, result: dict[str, Any], summary: dict[str, Any]) -> None:
    ...
    try:
        ...
        summary["errors"].append(f"Failed to ingest NCR result: {exc}")


def run_ncr_agent() -> dict[str, Any]:
    api = FullpitchAPI()
    ...
    try:
        ...
        summary["errors"].append(f"Failed to fetch NCR schedule: {exc}")

    logger.info("NCR agent summary: %s", summary)
    return summary
