...
def _fetch_wer_fixtures() -> list[dict[str, Any]]:
    soup = fetch_narugbydb_html(WER_FIXTURES_URL)
    matches = parse_fixtures_page(soup)
    logger.info("Parsed %d WER fixtures/results from NA Rugby DB", len(matches))
    return matches


def _fetch_wer_standings() -> list[dict[str, Any]]:
    soup = fetch_narugbydb_html(WER_STANDINGS_URL)
...
def run_wer_agent() -> dict[str, Any]:
    ...
    try:
        _ingest_matches(api, season, _fetch_wer_fixtures(), summary)
    ...
    try:
        _ingest_standings(api, season, _fetch_wer_standings(), summary)
    except ScraperError as exc:
