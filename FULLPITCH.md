...
| Field | Valid Values |
|-------|-------------|
| `league` | `mlr` `wer` `college` `craa-d1a` `craa-d1aa` `craa-women` `ncr-d1` `ncr-d2` `ncr-d3` `ncr-women` `nira` `club` `high-school` `eagles` `community` `world` `general` |
...
│   ├── ncr_agent.py         ← NCR news and results
│   └── nira_agent.py        ← NIRA women's college news and results
├── tools/
...
| `ncr-agent` | Every 2 hours | NCR news and results |
| `nira-agent` | Every 2 hours | NIRA women's college news and results |
...
- **Step 15 (roadmap):** Skipped intentionally — the existing Railway service runs `python main.py` continuously; Vercel cron jobs are not needed.
...
- **Editorial cleanup and summary budget:** `tools/editorial_ai.py` centralizes `shorten_title()` and `normalize_feed_summary()`. News, CRAA, NCR, NIRA, and match-report article ingests shorten long titles to 5-7 words before posting. The shared `prompts/article_summary.txt` and `tools/scraper.gemini_summarize()` now request one tight paragraph, max 60 words, no filler intro phrases, and no "The" opener. NCR article discovery only ingests URLs whose path contains `/news/`. Fast maintenance now regenerates only missing, over-100-word, multi-paragraph, or filler-opening summaries, capped at 10 attempts per run, and runs a separate `fast_title_shortening` task that scans all existing articles with titles over 7 words, uses Gemini to shorten up to 15 titles per run, logs `Title shortened: old → new`, and deletes Student Leader / Student-Leader / Powered by GDPR junk articles; full maintenance deletes junk NCR/admin-style article titles containing Student Leader of the Month, Membership Dues, Coaching Mentorship Program, Broadcast Academy, FirstPoint USA Official, Student-Leader, or Powered by GDPR.
- **College source agents:** `boss_agent.py` now runs MLR, WER, news, video, Eagles, existing college, HS diagnostics, CRAA, NCR, NIRA, then maintenance. `agents/craa_agent.py`, `agents/ncr_agent.py`, and `agents/nira_agent.py` discover URLs from `/api/v1/sources`, skip navigation/non-article URLs with shared filters, fetch full article pages, extract `og:image`/`publishedDate`, use batched relevance checks where applicable, and ingest college articles with the most specific league tag possible (`craa-d1a`, `craa-d1aa`, `craa-women`, `ncr-d1`, `ncr-d2`, `ncr-d3`, `ncr-women`, `nira`, or `college`) via shared `tools/college_leagues.py` rules plus Gemini fallback. `video_agent.py` uses the same college-tag helper for YouTube titles/channels, decodes HTML entities in video titles/descriptions/channel names before ingest, and now only accepts returned YouTube videos whose `channelTitle` partially matches an approved source-channel name such as Major League Rugby, USA Rugby, CRAA Rugby, National Collegiate Rugby, The Rugby Review, The Jacks Rangers Show, or TJRS; unapproved channels are rejected without Gemini. CRAA resolves postseason results as `league="craa-d1a"`; NCR and NIRA parse conservative score lines and skip matches when teams cannot be resolved. NIRA sends `gender="WOMENS"` where the app ingest API supports Article gender.
- **Browser-rendered source diagnostics:** Agents service now includes Playwright (`playwright>=1.40.0`) and a Dockerfile that installs Chromium/system libraries for Railway Dockerfile builds. `tools/browser.py` provides `fetch_js_page()` for JS-rendered pages while blocking images/fonts, and `tools/schedule.py` gates expensive browser tasks to weekends. `wer_agent.py` continues using NA Rugby DB for WER fixtures but only launches Playwright on weekends for `https://www.womenseliterugby.us/standings`, logging the first rendered text and context around each known WER team for parser tuning. New `agents/hs_agent.py` runs on weekends only, fetches Xplorer Girls Nationals results with Playwright, and logs score-candidate contexts; `boss_agent.py` and `main.py --agent hs` wire it into the service.
...
| May 2026 | Steps 14 and 15 skipped — ADK agents on Railway handle fetching inside the existing long-running agents service; Vercel scraper routes/crons are not needed |
| May 2026 | Existing Railway agents service now runs continuously: `main.py` starts a daemon live-score thread beside the hourly boss loop; the cron schedule was removed from `railway.json`. |
...
| May 2026 | Maintenance agent now regenerates article summaries shorter than 150 words with `gemini_summarize()` so legacy short snippets become 150-200 word summaries. |
...
| May 20, 2026 | Live score regression guard: MLR and WER agents reject screenshot/parser scores lower than DB (`homeScore`/`awayScore` only increase during live matches). |
