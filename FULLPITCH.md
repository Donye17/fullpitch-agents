...
- **Maintenance agent:** `agents/maintenance_agent.py` runs as the 8th boss sub-agent before CRAA ingest. Fast mode runs every cycle against articles created in the last 48 hours, prioritizing articles with missing `imageUrl` before other repairs, repairing missing `imageUrl`, `publishedDate`, `slug`, `summary`, `sourceDomain`, HTML-escaped titles, and wrong `league` tags; it also deletes known junk nav-page titles through protected `DELETE /api/v1/articles/[id]`. Full mode runs only at 3 AM UTC, fetches all articles, applies the same repairs across history, and summaries under 150 words regenerate with `gemini_summarize()`.
