...
- **Maintenance agent:** `agents/maintenance_agent.py` runs as the final boss sub-agent after source ingest. Fast mode runs every cycle against articles created in the last 48 hours, prioritizing articles with missing `imageUrl` before other repairs, repairing missing `imageUrl`, `publishedDate`, `slug`, `summary`, `sourceDomain`, HTML-escaped titles, and wrong `league` tags; it also deletes known junk nav-page titles through protected `DELETE /api/v1/articles/[id]`. Full mode runs only at 3 AM UTC, fetches all articles, applies the same repairs across history, regenerates summaries under 150 words with `gemini_summarize()`, and decodes HTML entities in existing video titles through protected `PATCH /api/v1/videos/[id]`.
- **College source agents:** `boss_agent.py` now runs MLR, WER, news, video, Eagles, existing college, CRAA, NCR, NIRA, then maintenance. `agents/craa_agent.py`, `agents/ncr_agent.py`, and `agents/nira_agent.py` discover URLs from `/api/v1/sources`, skip navigation/non-article URLs with shared filters, fetch full article pages, extract `og:image`/`publishedDate`, run Gemini relevance/classification checks, and ingest college articles with the most specific league tag possible (`craa-d1a`, `craa-d1aa`, `craa-women`, `ncr-d1`, `ncr-d2`, `ncr-d3`, `ncr-women`, `nira`, or `college`) via shared `tools/college_leagues.py` rules plus Gemini fallback. `video_agent.py` uses the same college-tag helper for YouTube titles/channels and decodes HTML entities in video titles/descriptions/channel names before ingest. CRAA resolves postseason results as `league="craa-d1a"`; NCR and NIRA parse conservative score lines and skip matches when teams cannot be resolved. NIRA sends `gender="WOMENS"` where the app ingest API supports Article gender.
